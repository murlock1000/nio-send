...
reply_regex = re.compile(
    r"<mx-reply><blockquote>.*</blockquote></mx-reply>(.*)", flags=re.RegexFlag.DOTALL
)
...
def get_reply_msg(
    event: nio.Event, reply_to: Optional[str], replaces: Optional[str]
) -> Optional[str]:
...
async def get_room_id(
    client: nio.AsyncClient, room: str, logger: logging.Logger
) -> str:
    if room.startswith("#"):
